...
exports.isToastlyConfigured = isToastlyConfigured;
exports.isContainerPresent = isContainerPresent;
 ...
 */
function detectProjectType(tree, project) {
    const sourceRoot = (0, workspace_1.getSourceRoot)(project);
 ...
 */
function findAppConfigPath(tree, mainFilePath) {
    const mainContent = tree.read(mainFilePath);
    if (!mainContent)
        return undefined;
    const mainText = mainContent.toString('utf-8');
 ...
 */
function isToastlyConfigured(tree, filePath) {
 ...
 */
function isContainerPresent(tree, componentPath) {
 ...
 */
function resolveTemplatePath(componentPath, templateUrl) {
