...
export function addContainer(
  ...
  if (!tree.exists(componentPath)) {
    throw new SchematicsException(
      `Could not find AppComponent at ${componentPath}. ` +
  ...
  if (!content) {
    throw new SchematicsException(`Could not read ${componentPath}`);
  }
 ...
 */
function addContainerToExternalTemplate(
  tree: Tree,
  ...
  if (!tree.exists(templatePath)) {
    throw new SchematicsException(
      `Could not find template at ${templatePath}. ` +
      'Please add <toastly-container /> to your app template manually.'
    );
  }
  ...
  if (!content) {
    throw new SchematicsException(`Could not read ${templatePath}`);
  }
 ...
 */
function addContainerToInlineTemplate(
  tree: Tree,
  componentPath: string,
  text: string
): void {
  ...
  if (!templateMatch) {
    throw new SchematicsException(
      'Could not find template in AppComponent. ' +
      'Please add <toastly-container /> to your app template manually.'
    );
  }
 ...
 */
function addImportToComponent(tree: Tree, componentPath: string): void {
  const content = tree.read(componentPath);
  if (!content) return;
 ...
 */
function addToastlyImport(text: string): string {
  // Check if already imported
  if (text.includes('ToastContainerComponent')) {
    return text;
  }
